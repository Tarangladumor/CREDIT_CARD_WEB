...
import { addCardHowToApplyDetails } from '../../services/Operations/cardAPI'; // Adjust import path
...
//   const { fields: listDataFields, append: appendListData } = useFieldArray({ control, name: "listData" });
  ...
  const onSubmit = async (data) => {
    ...
    //   console.log("FormData before API Call:", Array.from(formData.entries())); // Debugging log
      ...
      const response = await addCardHowToApplyDetails(formData, token);

      if (response?.success) {
        toast.success('HOWTOAPPLY details added successfully!');
        navigate(`/addBenefits?token=${token}`); // Replace with actual path after form submission
      }
    } catch (error) {
      console.error('Error adding HOWTOAPPLY details:', error);
      toast.error('Failed to add HOWTOAPPLY details.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="bg-teal-50 min-h-screen flex items-center justify-center">
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white shadow-md rounded-lg p-8 max-w-md w-full">
        <h2 className="text-2xl font-bold text-teal-600 mb-6">Create a How To Apply</h2>
