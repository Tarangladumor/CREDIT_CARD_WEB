          ...
          .map(([key, value]) => (
            <SwiperSlide key={key}>
              <div className="bg-[#D2F4E4] rounded-tl-3xl rounded-br-3xl p-10 md:p-8 h-[400px] flex flex-col gap-3 shadow-[rgba(0,_0,_0,_0.2)_0px_60px_40px_-7px] mb-24">
                ...
                {/* Content inside the card with overflow scroll if exceeds the height */}
                <section className="overflow-y-auto max-h-[250px]">
                  <ul className="list-disc list-inside px-10 flex flex-col gap-y-5">
                    {value.map((item, index) =>
                      Array.isArray(item.listData) && item.listData.length > 0
                        ? item.listData.map((listItem, subIndex) => (
                            listItem.trim() && ( // Ensure that listItem is not empty or whitespace
